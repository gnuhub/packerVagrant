...
bash "Configure TDS WAS App" do
	cwd "/opt/ibm/ldap/V6.3/idstools/"
	user "root"
	group "root"
	code <<-EOH
		#echo "Configure TDS WAS App"
		/installFiles/tds63-linux-x86-64/appsrv/install.sh -installRoot /opt/TDS_WAS/
		./deploy_IDSWebApp -w /opt/ibm/ldap/V6.3/idstools/IDSWebApp.war -p /opt/TDS_WAS/
		EOH
end
...
bash "Enable services autostart" do
	...
	code <<-EOH
		...
		EOH
		ignore_failure false
		not_if do ::File.exists?('/etc/rc.d/rc5.d/S99WASServer') end
end
...
service "WASServer.sh" do
  action	:start
  not_if "ps auxww | grep TDSWebAdminProfile | grep -v grep"
end